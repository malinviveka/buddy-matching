...
def gale_shapley(students, buddies, student_preferences, buddy_preferences):
    ...
    unmatched_students = deque(
        students
    )  # Deque of students who have not yet been matched

    matches = defaultdict(list)  # Buddy -> [Students]

    while unmatched_students:
        student = unmatched_students.popleft()  # get the first unmatched student
        student_pref_list = student_preferences[
            student
        ]  # get the preference list of the student
        ...
        for buddy, _ in student_pref_list:
            ...
            lowest_score_student = min(
                matches[buddy],
                key=lambda s: next(
                    score for b, score in buddy_preferences[buddy] if b == s
                ),
            )
            new_score = next(
                score for b, score in buddy_preferences[buddy] if b == student
            )
            current_lowest_score = next(
                score
                for b, score in buddy_preferences[buddy]
                if b == lowest_score_student
            )
...
def run_matching():
    # load data
    buddies = (
        BuddyMatchingUser.objects.filter(
            role="Buddy", is_permitted=True, is_staff=False
        )
        .annotate(partner_count=Count("partners"))
        .filter(preferred_number_of_partners__gte=F("partner_count"))
    )
    students = (
        BuddyMatchingUser.objects.filter(
            role="International Student", is_permitted=True, is_staff=False
        )
        .annotate(partner_count=Count("partners"))
        .filter(partner_count=0)
    )
