...
class FeedbackView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            feedback = form.save(
                commit=False
            )  # Create a feedback object without saving it
            if isinstance(request.user, BuddyMatchingUser):
                feedback.student_email = request.user.email  # Set the `student` field
            else:
                return JsonResponse(
                    {"error": "User is not a valid BuddyMatchingUser"}, status=400
                )
...
def admin_feedback_list(request):
    ...
    return render(
        request, "feedback/admin_feedback_list.html", {"feedbacks": feedbacks}
    )
...
def export_feedback_csv(request):
    ...
    )  # add 'Student' in the beginning, if feedback should not be ananomous
    ...
    for feedback in feedbacks:
        writer.writerow(
            [
                ...
                feedback.q9_details,
                feedback.q10,
                feedback.submitted_at.strftime(
                    "%Y-%m-%d"
                ),  # if time should be included: '%Y-%m-%d %H:%M:%S'
            ]
        )
