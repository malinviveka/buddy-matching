...
class FeedbackForm(forms.ModelForm):
    class Meta:
        model = Feedback
        fields = ['q1', 'q2', 'q3', 'q4', 'q5', 'q5_details', 'q6', 'q7', 'q8', 'q8_details', 'q9', 'q9_details', 'q10']
        widgets = {
            'q1': forms.Select(attrs={'id': 'id_q1'}, choices=Feedback.ROLE_CHOICES),
            'q2': forms.Select(attrs={'id': 'id_q2'}, choices=[(i, str(i)) for i in range(1, 6)]),
            'q3': forms.Select(attrs={'id': 'id_q3'}, choices=[(i, str(i)) for i in range(1, 6)]),
            'q4': forms.Select(attrs={'id': 'id_q4'}, choices=Feedback.DISCOVERY_CHOICES),
            'q5': forms.Select(attrs={'id': 'id_q5'}, choices=Feedback.SUPPORT_CHOICES),
            'q5_details': forms.Textarea(attrs={'id': 'id_q5_details', 'rows': 4, 'cols': 40}),
            'q6': forms.Select(attrs={'id': 'id_q6'}, choices=Feedback.FIRST_CONTACT_CHOICES),
            'q7': forms.Select(attrs={'id': 'id_q7'}, choices=Feedback.CONTACT_CHOICES),
            'q8': forms.Select(attrs={'id': 'id_q8'}, choices=Feedback.PROBLEMS_CHOICES),
            'q8_details': forms.Textarea(attrs={'id': 'id_q8_details', 'rows': 4, 'cols': 40}),
            'q9': forms.Select(attrs={'id': 'id_q9'}, choices=Feedback.RECOMMENDATION_CHOICES),
            'q9_details': forms.Textarea(attrs={'id': 'id_q9_details', 'rows': 4, 'cols': 40}),
    ...
    def clean(self):
        ...
        if q9 == 'No' and not q9_details:
            self.add_error('q9_details', 'Would you like to explain us why you would not recommend us?')
